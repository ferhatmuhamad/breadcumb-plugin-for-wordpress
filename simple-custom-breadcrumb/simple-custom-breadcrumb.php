...
function scb_enqueue_assets()
{
	...
	$align         = in_array(isset($opts['align']) ? $opts['align'] : 'left', array('left', 'center', 'right'), true)
		? $opts['align'] : 'left';
...
function scb_shortcode($atts)
{
	...
	// Sanitize align: allowlist.
	if ('' !== $atts['align']) {
		$allowed_align = array('left', 'center', 'right');
		if (in_array($atts['align'], $allowed_align, true)) {
			$overrides['align'] = $atts['align'];
		}
	}
